...
/*****
 * 本文件提供了一个从组件创建蹊径落地页(Xijing page)的简单示例
 */
class AddXijingPageByComponents
{
    public static $tads;
    ...
    public static $IMAGE_ID     = 'YOUR BACKGROUND IMAGE ID'; // 背景图片ID
    ...
    public function main()
    {
        try {
            ...
            $response = $tads->xijingPageByComponents()
                             ->add([
...
if (basename($_SERVER['PHP_SELF']) == basename(__FILE__)) {
    try {
        $example = new AddXijingPageByComponents();
        $example->init();
        $example->main();
    } catch (\Exception $e) {
        exit(-1);
