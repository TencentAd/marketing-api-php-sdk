...
/*****
 * 本文件提供了一个更新线索(Outer clues)基本信息的简单示例
 */
class UpdateOuterCluesContact
{
    public static $tads;
    public static $ACCESS_TOKEN   = 'YOUR ACCESS TOKEN';
    public static $ACCOUNT_ID     = 'YOUR ACCOUNT ID';
    public static $MATCH_TYPE     = 'OUTERLEADSID';
    ...
    public function main()
    {
        try {
            ...
            $response = $tads->outerCluesContact()
                             ->update([
...
if (basename($_SERVER['PHP_SELF']) == basename(__FILE__)) {
    try {
        $example = new UpdateOuterCluesContact();
        $example->init();
        $example->main();
    } catch (\Exception $e) {
        exit(-1);
