...
/*****
 * 本文件提供了一个资金合并类型同商务管家子客间转账(Merge Fund Type Subcustomer transfer)的简单示例
 */
class AddMergeFundTypeSubcustomerTransfer
{
    public static $tads;
    ...
    public function main()
    {
        try {
            ...
            $response = $tads->mergeFundTypeSubcustomerTransfer()
                             ->add([
...
if (basename($_SERVER['PHP_SELF']) == basename(__FILE__)) {
    try {
        $example = new AddMergeFundTypeSubcustomerTransfer();
        $example->init();
        $example->main();
    } catch (\Exception $e) {
        exit(-1);
