...
/*****
 * 本文件提供了一个获取资金合并类资金账户日结明细(Merge Fund Type Daily balance report)列表的简单示例
 */
class GetMergeFundTypeDailyBalanceReport
{
    public static $tads;
    ...
    public function main()
    {
        try {
            ...
            $response = $tads->mergeFundTypeDailyBalanceReport()
                ->get([
...
if (basename($_SERVER['PHP_SELF']) == basename(__FILE__)) {
    try {
        $example = new GetMergeFundTypeDailyBalanceReport();
        $example->init();
        $example->main();
    } catch (\Exception $e) {
        exit(-1);
