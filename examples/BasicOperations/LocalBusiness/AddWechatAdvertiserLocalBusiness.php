...
/*****
 * 本文件提供了一个创建附近推商家(Wechat Advertiser Local Business)的简单示例
 */
class AddWechatAdvertiserLocalBusiness
{
    public static $tads;
    ...
    public static $BUSINESS_ID            = 'YOUR BUSINESS ID'; // 商家ID
        ...
        try {
            ...
            $response = $tads->wechatAdvertiserLocalBusiness()
                             ->add([
                                 ...
                                 'business_id'            => static::$BUSINESS_ID,
                             ]);
...
if (basename($_SERVER['PHP_SELF']) == basename(__FILE__)) {
    try {
        $example = new AddWechatAdvertiserLocalBusiness();
        $example->init();
        $example->main();
    } catch (\Exception $e) {
        exit(-1);
