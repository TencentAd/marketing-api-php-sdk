    ...
    public function main()
    {
        try {
            ...
            $fields = [
                ...
                'adcreative_template_size', 'adcreative_template_style', 'adcreative_template_appellation',
                'site_set', 'promoted_object_type', 'adcreative_sample_image_list', 'ad_attributes',
                'adcreative_attributes', 'adcreative_elements', 'support_page_type', 'support_billing_spec_list',
                'support_dynamic_ability_spec_list',
            ]; // 需要返回的字段
            $response = $tads->adcreativeTemplates()
                             ->get([
...
if (basename($_SERVER['PHP_SELF']) == basename(__FILE__)) {
    try {
        $example = new GetAdcreativeTemplates();
        $example->init();
        $example->main();
    } catch (\Exception $e) {
        exit(-1);
