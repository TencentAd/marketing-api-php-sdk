 ...
 */
class AddWechatMomentAdsWithLinkPage
{
    public static $tads;
    ...
    public static $WECHAT_PAGE_ID         = 'YOUR WECHAT PAGE ID'; // 原生页ID
    public static $AD_TITLE               = 'YOUR AD TEXT'; // 广告文案，310规格要求：字数：1~40
    public static $PAGE_TYPE              = 'PAGE_TYPE_CANVAS_WECHAT'; // 外层落地页：原生页
    public static $LINK_PAGE_TYPE         = 'LINK_PAGE_TYPE_CANVAS_WECHAT'; // 行动按钮跳转落地页：原生页
    ...
    public function main()
    {
        try {
            ...
            // 第二步，创建推广目标
            $promotedObjectId = $this->AddPromotedObject($tads, $promotedObjectType, static::$ANDROID_APP_ID);
    ...
    private function AddAdcreative(
        ...
        $pageSpec = [
            'page_id'                     => $wechatPageId,
            'override_canvas_head_option' => 'OPTION_CANVAS_OVERRIDE_CREATIVE',
        ];
        $linkPageSpec = [
            'page_id' => $wechatPageId,
        ];
...
if (basename($_SERVER['PHP_SELF']) == basename(__FILE__)) {
    try {
        $example = new AddWechatMomentAdsWithLinkPage();
        $example->init();
        $example->main();
    } catch (\Exception $e) {
        exit(-1);
