# 更新日志
本文记录了PHP SDK的更新日志。

## 2026-02-06
1. 发布PHP SDK [1.1.93] 版
- 新增 MarketingApi 接口，更新模型

## 2025-01-22
1. 发布PHP SDK [1.1.92] 版
- 新增 MarketingApi 接口，更新模型

## 2025-01-12
1. 发布PHP SDK [1.1.91] 版
- 新增 MarketingApi 接口，更新模型

## 2025-12-03
1. 发布PHP SDK [1.1.90] 版
- 新增 MarketingApi 接口，更新模型

## 2025-11-05
1. 发布PHP SDK [1.1.89] 版
- 新增 MarketingApi 接口，更新模型

## 2025-08-20
1. 发布PHP SDK [1.1.88] 版
- 新增 MarketingApi 接口，更新模型

## 2025-08-06
1. 发布PHP SDK [1.1.87] 版
- 新增 MarketingApi 接口，更新模型
- 支持user_token参数

## 2025-07-02
1. 发布PHP SDK [1.1.86] 版
- 新增 MarketingApi 接口，更新模型

## 2025-06-20
1. 发布PHP SDK [1.1.85] 版
- 新增 MarketingApi 接口，更新模型

## 2025-05-28
1. 发布PHP SDK [1.1.84] 版
- 新增 MarketingApi 接口，更新模型

## 2025-04-11
1. 发布PHP SDK [1.1.83] 版
- 新增 MarketingApi 接口，更新模型

## 2025-02-27
1. 发布PHP SDK [1.1.82] 版
- 新增 MarketingApi 接口，更新模型
- 随着旧指标体系的下线，广告 2.0 数据洞察报表接口不再支持旧指标体系的指标
- 修复1.1.81版本campaign_id等业务指标被误删除的问题

## 2025-02-27
1. 发布PHP SDK [1.1.81] 版
- 新增 MarketingApi 接口，更新模型
- 随着旧指标体系的下线，广告 2.0 数据洞察报表接口不再支持旧指标体系的指标

## 2025-02-12
1. 发布PHP SDK [1.1.80] 版
- 新增 MarketingApi 接口，更新模型

## 2025-01-02
1. 发布PHP SDK [1.1.79] 版
- 新增 MarketingApi 接口，更新模型

## 2024-12-19
1. 发布PHP SDK [1.1.78] 版
- 新增 MarketingApi 接口，更新模型
- 修复日报表接口获取不到数据的问题

## 2024-12-16
1. 发布PHP SDK [1.1.77] 版
- 新增 MarketingApi 接口，更新模型

## 2024-12-03
1. 发布PHP SDK [1.1.76] 版
- 新增 MarketingApi 接口，更新模型

## 2024-10-16
1. 发布PHP SDK [1.1.75] 版
- 新增 MarketingApi 接口，更新模型

## 2024-07-13
1. 发布PHP SDK [1.1.74] 版
- 新增 MarketingApi 接口，更新模型

## 2024-07-02
1. 发布PHP SDK [1.1.73] 版
- 新增 MarketingApi 接口，更新模型

## 2024-06-04
1. 发布PHP SDK [1.1.72] 版
- 新增 MarketingApi 接口，更新模型

## 2024-05-23
1. 发布PHP SDK [1.1.71] 版
- 新增 MarketingApi 接口，更新模型

## 2024-05-16
1. 发布PHP SDK [1.1.70] 版
- 新增 MarketingApi 接口，更新模型
- 修复 v2和v3 TencentAds 混合初始化的问题

## 2024-05-11
1. 发布PHP SDK [1.1.69] 版
- 新增 MarketingApi 接口，更新模型

## 2024-04-26
1. 发布PHP SDK [1.1.68] 版
- 新增 MarketingApi 接口，更新模型

## 2024-04-11
1. 发布PHP SDK [1.1.67] 版
- 新增 MarketingApi 接口，更新模型

## 2024-04-02
1. 发布PHP SDK [1.1.66] 版
- 新增 MarketingApi 接口，更新模型

## 2024-03-30
1. 发布PHP SDK [1.1.65] 版
- 新增 MarketingApi 接口，更新模型
- 修复部分Get接口参数转换问题

## 2024-03-29
1. 发布PHP SDK [1.1.64] 版
- 新增 MarketingApi 接口，更新模型
- 修复部分接口数组参数自动转换为字符串的问题

## 2024-03-29
1. 发布PHP SDK [1.1.63] 版
- 新增 MarketingApi 接口，更新模型

## 2024-03-15
1. 发布PHP SDK [1.1.62] 版
- 新增 MarketingApi 接口，更新模型

## 2024-03-13
1. 发布PHP SDK [1.1.61] 版
- 新增 MarketingApi 接口，更新模型

## 2024-02-22
1. 发布PHP SDK [1.1.60] 版
- 新增 MarketingApi 接口，更新模型

## 2024-02-20
1. 发布PHP SDK [1.1.59] 版
- 新增 MarketingApi 接口，更新模型

## 2024-01-26
1. 发布PHP SDK [1.1.58] 版
- 新增 MarketingApi 接口，更新模型
- 修复报表接口返回类型缺少部分指标字段的问题

## 2024-01-18
1. 发布PHP SDK [1.1.57] 版
- 增加新接口支持
- 增加MKTAPI 3.0 接口支持

## 2023-10-18
1. 发布PHP SDK [1.1.56] 版
- 增加新接口支持

## 2023-09-01
1. 发布PHP SDK [1.1.55] 版
- 增加新接口支持

## 2023-08-30
1. 发布PHP SDK [1.1.54] 版
- 增加新接口支持

## 2023-05-09
1. 发布PHP SDK [1.1.52] 版
- 增加新接口支持

## 2023-03-31
1. 发布PHP SDK [1.1.51] 版
- 增加新接口支持

## 2023-02-27
1. 发布PHP SDK [1.1.50] 版
- 增加新接口支持

## 2022-12-05
1. 发布PHP SDK [1.1.49] 版
- 增加新接口支持

## 2022-10-25
1. 发布PHP SDK [1.1.48] 版
- 增加新接口支持

## 2022-10-10
1. 发布PHP SDK [1.1.46] 版
- 增加新接口支持

## 2022-08-04
1. 发布PHP SDK [1.1.42] 版
- 增加新接口支持

## 2022-07-25
1. 发布PHP SDK [1.1.40] 版
- 增加新接口支持

## 2022-06-23
1. 发布PHP SDK [1.1.39] 版
- 增加新接口支持

## 2022-05-09
1. 发布PHP SDK [1.1.38] 版
- 增加新接口支持
- 支持guzzlehttp/guzzle 7.3以上版本

## 2022-04-27
1. 发布PHP SDK [1.1.37] 版
- 增加新接口支持

## 2022-03-25
1. 发布PHP SDK [1.1.36] 版
- 增加新接口支持

## 2022-02-23
1. 发布PHP SDK [1.1.35] 版
- 增加新接口支持

## 2022-01-25
1. 发布PHP SDK [1.1.34] 版
- 增加新接口支持

## 2021-12-30
1. 发布PHP SDK [1.1.33] 版
- 增加新接口支持

## 2021-12-16
1. 发布PHP SDK [1.1.32] 版
- 增加新接口支持
- 修复返回数组模式下批量接口及快速创建接口不兼容的问题

## 2021-11-19
1. 发布PHP SDK [1.1.31] 版
- 增加新接口支持

## 2021-10-08
1. 发布PHP SDK [1.1.30] 版
- 增加新接口支持

## 2021-08-31
1. 发布PHP SDK [1.1.29] 版
- 增加新接口支持
- 修复async_report_files/get接口返回解析异常的问题

## 2021-08-16
1. 发布PHP SDK [1.1.28] 版
- 增加新接口支持

## 2021-08-05
1. 发布PHP SDK [1.1.27] 版
- 修复PHPDoc中@param字段类型不准确的问题

## 2021-08-04
1. 发布PHP SDK [1.1.26] 版
- 增加新接口支持
- 限制guzzlehttp/guzzle不支持7.3以上版本
- 修复PHPDoc中@return字段类型不准确的问题

## 2021-07-27
1. 发布PHP SDK [1.1.25] 版
- 增加新接口支持

## 2021-07-07
1. 发布PHP SDK [1.1.24] 版
- 增加新接口支持

## 2021-06-21
1. 发布PHP SDK [1.1.23] 版
- 增加新接口支持及代码示例补充
- 修复动态创意接口中image_list_options字段类型不正确的问题

## 2021-05-28
1. 发布PHP SDK [1.1.22] 版
- 增加新接口支持
- 修复蹊径落地页接口返回结构无法解析的问题

## 2021-05-19
1. 发布PHP SDK [1.1.21] 版
- 增加新接口支持

## 2021-04-30
1. 发布PHP SDK [1.1.20] 版
- 增加新接口支持

## 2021-04-15
1. 发布PHP SDK [1.1.19] 版
- 增加新接口支持

## 2021-03-31
1. 发布PHP SDK [1.1.18] 版
- 增加新接口支持

## 2021-03-16
1. 发布PHP SDK [1.1.17] 版
- 增加新接口支持

## 2021-03-01
1. 发布PHP SDK [1.1.16] 版
- 增加新接口支持

## 2021-02-22
1. 发布PHP SDK [1.1.15] 版
- 增加新接口支持
- 去除PHP7语法，以保证可在PHP5.5下运行

## 2021-02-03
1. 发布PHP SDK [1.1.14] 版
- 增加新接口支持

## 2021-01-19
1. 发布PHP SDK [1.1.13] 版
- 增加新接口支持

## 2021-01-06
1. 发布PHP SDK [1.1.12] 版
- 增加新接口支持

## 2020-12-22
1. 发布PHP SDK [1.1.11] 版
- 增加新接口支持

## 2020-12-09
1. 发布PHP SDK [1.1.10] 版
- 增加新接口支持

## 2020-11-24
1. 发布PHP SDK [1.1.9] 版
- 增加新接口支持

## 2020-11-05
1. 发布PHP SDK [1.1.8] 版
- 增加新接口支持

## 2020-09-29
1. 发布PHP SDK [1.1.7] 版
- 增加新接口支持
- 增加新接口示例
- 调整朋友圈广告创建用例，反映双落地页参数

## 2020-09-11
1. 发布PHP SDK [1.1.6] 版
- 增加新接口支持
- 修复推广目标相关接口genres字段返回和IDL不符的问题
- 修复上传图片接口用UPLOAD_TYPE_BYTES方式上传的问题

## 2020-07-24
1. 发布PHP SDK [1.1.5] 版
- 一些问题修复

## 2020-07-17
1. 发布PHP SDK [1.1.4] 版
- 增加一次性创建计划-广告组-创意-广告的4合1接口
- 一些问题修复

## 2020-07-07
1. 发布PHP SDK [1.1.3] 版
- 修复示例中的问题
- 增加商品广告示例

## 2020-05-18
1. 发布PHP SDK [1.1.2] 版
- 兼容PHP7.2及以上版本

## 2020-05-08
1. 发布PHP SDK [1.1.1] 版
- 新增接口示例
- 支持marketing api最新接口

## 2020-04-02
1. 发布PHP SDK [1.1.0] 版
- 新增批量接口
- 新增场景示例

## 2020-02-29
1. 发布PHP SDK [1.0.0] 版
- 账号管理
- 营销资产
- 广告管理
- 人群管理
- 数据洞察
- 异步任务
