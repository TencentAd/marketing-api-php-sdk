<?php
/**
 * CatalogType
 *
 * PHP version 5
 *
 * @category Class
 * @package  TencentAds
 * @author   Swagger Codegen team
 * @link     https://github.com/swagger-api/swagger-codegen
 */

/**
 * Marketing API
 *
 * Marketing API
 *
 * OpenAPI spec version: 1.3
 * 
 * Generated by: https://github.com/swagger-api/swagger-codegen.git
 * Swagger Codegen version: 2.4.13
 */

/**
 * NOTE: This class is auto generated by the swagger code generator program.
 * https://github.com/swagger-api/swagger-codegen
 * Do not edit the class manually.
 */

namespace TencentAds\Model;
use \TencentAds\ObjectSerializer;

/**
 * CatalogType Class Doc Comment
 *
 * @category Class
 * @description 商品库类型
 * @package  TencentAds
 * @author   Swagger Codegen team
 * @link     https://github.com/swagger-api/swagger-codegen
 */
class CatalogType
{
    /**
     * Possible values of this enum
     */
    const CATALOG_TYPE_STANDARD = 'CATALOG_TYPE_STANDARD';
    const CATALOG_TYPE_FENGYE = 'CATALOG_TYPE_FENGYE';
    const CATALOG_TYPE_LOCAL_STORE = 'CATALOG_TYPE_LOCAL_STORE';
    const CATALOG_TYPE_YOUZAN = 'CATALOG_TYPE_YOUZAN';
    const CATALOG_TYPE_WEIMENG = 'CATALOG_TYPE_WEIMENG';
    const CATALOG_TYPE_MADAO = 'CATALOG_TYPE_MADAO';
    const PRODUCT_CATALOG_TYPE_STANDARD = 'PRODUCT_CATALOG_TYPE_STANDARD';
    const PRODUCT_CATALOG_TYPE_FENGYE = 'PRODUCT_CATALOG_TYPE_FENGYE';
    const PRODUCT_CATALOG_TYPE_SMALLSHOP = 'PRODUCT_CATALOG_TYPE_SMALLSHOP';
    const PRODUCT_CATALOG_TYPE_YUEBAO = 'PRODUCT_CATALOG_TYPE_YUEBAO';
    const PRODUCT_CATALOG_TYPE_MAIBAO = 'PRODUCT_CATALOG_TYPE_MAIBAO';
    const PRODUCT_CATALOG_TYPE_YOUZAN = 'PRODUCT_CATALOG_TYPE_YOUZAN';
    const PRODUCT_CATALOG_TYPE_WEIMENG = 'PRODUCT_CATALOG_TYPE_WEIMENG';
    const PRODUCT_CATALOG_TYPE_MADAO = 'PRODUCT_CATALOG_TYPE_MADAO';
    const CATALOG_TYPE_UNKNOWN = 'CATALOG_TYPE_UNKNOWN';
    
    /**
     * Gets allowable values of the enum
     * @return string[]
     */
    public static function getAllowableEnumValues()
    {
        return [
            self::CATALOG_TYPE_STANDARD,
            self::CATALOG_TYPE_FENGYE,
            self::CATALOG_TYPE_LOCAL_STORE,
            self::CATALOG_TYPE_YOUZAN,
            self::CATALOG_TYPE_WEIMENG,
            self::CATALOG_TYPE_MADAO,
            self::PRODUCT_CATALOG_TYPE_STANDARD,
            self::PRODUCT_CATALOG_TYPE_FENGYE,
            self::PRODUCT_CATALOG_TYPE_SMALLSHOP,
            self::PRODUCT_CATALOG_TYPE_YUEBAO,
            self::PRODUCT_CATALOG_TYPE_MAIBAO,
            self::PRODUCT_CATALOG_TYPE_YOUZAN,
            self::PRODUCT_CATALOG_TYPE_WEIMENG,
            self::PRODUCT_CATALOG_TYPE_MADAO,
            self::CATALOG_TYPE_UNKNOWN,
        ];
    }
}


